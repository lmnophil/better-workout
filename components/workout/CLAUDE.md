...
The footer carries an optional **second action** when the caller passes `onPickAsPool`: once 2+ exercises are selected, an "Add N as a pool of alternatives" button appears beneath the plain add. Only the routine editor's add-to-day picker passes it; it adds the selection to the day and groups it into a new `TemplatePool` (default `pickCount` 1, adjusted afterward from the pool's row in the day's Pools panel — the lead exercise row shows the count read-only). The workout, swap, and add-to-existing-pool surfaces leave it unset, so no pool button shows there.
...
Above the search box are two rows of chips: regions (Upper / Lower / Full body / Mobility) and muscle groups (Chest / Back / Shoulders / Arms / Glutes / Quads / Hamstrings / Core), an equipment row, and a collapsible **module filter** (`Filter by module`, exact-match on `Exercise.module`, behind a toggle since the list is long). Selection is multi — chips are unioned with each other and with the search box; module chips AND against the rest. The "Full body" region chip is exclusive: tapping it clears every other chip. Tapping any non-Full chip cancels Full. The region/muscle chip taxonomy and filter logic live in `lib/area-filter.ts` so other surfaces (empty state, future coverage→picker links) can reuse them; the module filter is a plain inline string match.
