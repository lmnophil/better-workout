...
**Client components.** Mark with `'use client'`. Drive action calls through the `useAction` hook (`components/ui/use-action.tsx`): `run(() => action(...))` awaits inside a transition so `isPending` actually spans the request, surfaces an expected `{ ok: false }` in `error`, and recovers a rejection (offline/bug) inline instead of crashing to the boundary — don't hand-roll `startTransition` per call site, and never the statement form `startTransition(() => { action() })` (pending never engages). The exception is a dialog that must await-and-decide whether to close (`SaveTemplateDialog`) — it calls the action directly and reads `res.ok`. See [docs/api.md](docs/api.md) and the ADR in [docs/decisions.md](docs/decisions.md). For prefs that cross the layout/page boundary, use `PrefsContext` in `components/ui/prefs-context.tsx` — don't prop-drill prefs (see [docs/decisions.md](docs/decisions.md) for why).
